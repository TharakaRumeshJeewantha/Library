...
<?php
	...
	$issuings = mysqli_query($db, $query);

	if ($issuings) {
		while ($issuing = mysqli_fetch_assoc($issuings)) {
				
			$issue_lists .=	"<tr style='font-size: 13px;'>";
			$issue_lists .=	"<td>{$issuing['std_reg_no']} . {$issuing['std_fname']} {$issuing['std_lname']} ({$issuing['bk_no']} - {$issuing['bk_name']})</td>";
		 	$issue_lists .=	"<td> {$issuing['bk_issued_date']} - {$issuing['bk_should_return_date']} </td>";
			// $issue_lists .=	"<td>{$student['std_lname']}</td>";		
			// $issue_lists .=	"<td>{$student['std_gender']}</td>";
			$issue_lists .=	"<td>{$issuing['bk_returned_date']} ({$issuing['t_status']})</td>";
			// $issue_lists .=	"<td>{$student['std_class']}</td>";
			$issue_lists .=	"<td class='actions'>";

			$issue_lists .=	"<a href='#edit{$issuing['tid']}' data-toggle='modal' ><i class='fa fa-pencil'></i></a>";
			$issue_lists .=	"<a href='#del{$issuing['tid']}' data-toggle='modal' ><i class='fa fa-trash-o'></i></a>";
			include('return_fetch.php');
			$issue_lists .=	"</td>";
			$issue_lists .=	"</tr>";	
		}
	} else {
			...
			<div class="progress">
				...
				<section role="main" class="content-body">
					<header class="page-header">
						<h2>Edit / Delete Return Details</h2>
						...
						<div class="right-wrapper pull-right">
							<ol class="breadcrumbs">
								...
								<li><span>Update Return Details</span></li>
							</ol>
					...
					<!-- start: page -->
						...
						<section class="panel">
							<header class="panel-heading">
								...
								<h2 class="panel-title">Update Returning Details</h2>
							</header>
