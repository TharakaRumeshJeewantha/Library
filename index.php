...
<?php
	...
	$query4 = "SELECT * FROM tbl_day_time WHERE day = '$curr_day' AND status = 'yes'";
	$currents = mysqli_query($db, $query4);

	$count_current  = mysqli_num_rows($currents);

    if($count_current > 0) {
    	if ($currents) {
			while ($current = mysqli_fetch_assoc($currents)) {
														
			$current_grade_list .=	"<strong class='amount'>{$current['grade']} - {$current['class']}</strong>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;";
    	...
    	$current_grade_list .=  "<strong class='amount'>No Classes</strong>";
	}
?>
...
<?php
	...
	if ($max_reader['std_read'] == 0) {
		$max_reader['std_fname'] = "No";
		$max_reader['std_lname'] = "users";
		$max_reader['std_read'] = "No Users";
	}
	else {
		$max_reader['std_fname'];
		$max_reader['std_lname'];
		$max_reader['std_read'];
	}
...
<?php
	if ($faviourite_cat['bk_cat'] == 0) {
		$faviourite_cat['bk_category_no'] = "No users";
	}
	else {
		$faviourite_cat['bk_category_no'];
	}
?>
...
<?php
	...
	if ($best_grade['grade_readers'] == 0) {
		$best_grade['std_grade'] = "No users";
		$best_grade['grade_readers'] = "No users";
	}
	else {
		$best_grade['std_grade'];
		$best_grade['grade_readers'];
	}

?>

<?php
	$query_cost = "SELECT sum(bk_cost*bk_damages) as cost FROM tbl_books WHERE bk_damages <> 0";
	$result_cost = mysqli_query($db, $query_cost);
	$cost = mysqli_fetch_assoc($result_cost);

	if($cost['cost'] == 0) {
		$cost['cost'] = "0.00";
	}
	else {
		$cost['cost'] = $cost['cost'].".00";
	}
?>

<?php
	$query_dismiss = "SELECT COUNT(bk_id) as dismiss FROM tbl_books_damages WHERE reason = 'Dismiss'";
	$result_dismiss = mysqli_query($db, $query_dismiss);
	$dismiss = mysqli_fetch_assoc($result_dismiss);
?>

<?php
	$query_damage = "SELECT COUNT(bk_id) as damage_1 FROM tbl_books_damages WHERE reason = 'Damage'";
	$result_damage = mysqli_query($db, $query_damage);
	$damage_1 = mysqli_fetch_assoc($result_damage);
?>

<?php
	$query_other = "SELECT COUNT(bk_id) as other FROM tbl_books_damages WHERE reason = 'Other'";
	$result_other = mysqli_query($db, $query_other);
...
<?php
	...
	if($best_class['class_readers'] == 0) {
		$best_class['class_readers'] = "No users";
		$best_class['std_grade'] = "No";
		$best_class['std_class'] = "users";
		$best_class['teacher'] = "No users";

		$best_teacher['t_fname'] = "No";
		$best_teacher['t_lname'] = "users";
		$best_teacher['t_students'] = "No users";
	}
	else {

	$best_class['class_readers'];
	$best_class['std_grade'];
	$best_class['std_class'];
	$best_class['teacher'];
	$teacher_grade = $best_class['std_grade'];
	$teacher_class = $best_class['std_class'];

	$query13 = "SELECT t_fname, t_lname, t_students FROM tbl_teachers WHERE t_grade = '$teacher_grade' AND t_class = '$teacher_class' LIMIT 1";
...
<?php
	if(($girl['girls']+$boy['boys'])==0){
		$s_count = 1;
	}
	else {
		$s_count = ($girl['girls']+$boy['boys']);
	}
?>

<?php
	if($all_transactions['all_transactions'] == 0) {
		$s_trans = 1;
	}
	else {
		$s_trans = $all_transactions['all_transactions'];
	}
?>
...
<?php
	if($fav_book['fav_book'] == 0) {
		$fav_book['bk_name'] = "No users";
	}
	else {
		$fav_book['bk_name'];
	}
												...
												<div class="row text-center">
													<div class="col-md-3">
														<div class="circular-bar">
															<div class="circular-bar-chart" data-percent="<?= (($girl['girls'])/($s_count))*100 ?>" data-plugin-options='{ "barColor": "#F93AB9", "delay": 300 }'>
																<strong>Girls</strong>
																<label><span class="percent"><?= (($girl['girls'])/($s_count))*100 ?></span>%</label>
															</div>
														</div>
													</div>
													<div class="col-md-3">
														<div class="circular-bar">
															<div class="circular-bar-chart" data-percent="<?= (($boy['boys'])/($s_count))*100 ?>" data-plugin-options='{ "barColor": "#0088CC", "delay": 600 }'>
																<strong>Boys</strong>
																<label><span class="percent"><?= (($boy['boys'])/($s_count))*100 ?></span>%</label>
															</div>
														</div>
													</div>
													<div class="col-md-3">
									...
									<div class="panel-body bg-secondary">
										<div class="widget-summary">
											<div class="widget-summary-col widget-summary-col-icon">
												...
												<div class="widget-summary-col">
													<div class="summary">
														<h4 class="title">All Lost Books</h4>
															<div class="info">
																<strong class="amount"><?= $damage['damages'] ?></strong>
															</div>
													</div>
													<div class="summary-footer">
														<a class="text-uppercase">Damage: <?= $damage_1['damage_1'] ?> | Dismiss: <?= $dismiss['dismiss'] ?> | Other: <?= $other['other'] ?></a>
													</div>
												</div>
										</div>
									</div>
									...
									<div class="panel-body bg-tertiary">
										<div class="widget-summary">
											<div class="widget-summary-col widget-summary-col-icon">
												<div class="summary-icon">
													<div class="circular-bar circular-bar-xs m-none mt-xs mr-md">
														<div class="circular-bar-chart" data-percent="<?= ($status['status']/$s_trans)*100 ?>" data-plugin-options='{ "barColor": "#F9913A", "delay": 300, "size": 90, "lineWidth": 5 }'>
												...
												<div class="widget-summary">
													...
													<div class="widget-summary-col">
														<div class="summary">
															...
															<div class="info">
																<strong class="amount"><?= $best_grade['std_grade'] ?></strong>
															</div>
														</div>
														<div class="summary-footer">
												...
												<div class="widget-summary">
													...
													<div class="widget-summary-col">
														<div class="summary">
															...
															<div class="info">
																<strong class="amount"><?= $best_class['std_grade'] ?> - <?= $best_class['std_class'] ?></strong>
															</div>
														</div>
														<div class="summary-footer">
												...
												<div class="widget-summary">
													...
													<div class="widget-summary-col">
														<div class="summary">
															...
															<div class="info">
																<strong class="amount"><?= $best_teacher['t_fname'] ?> <?= $best_teacher['t_lname'] ?></strong>
															</div>
														</div>
														<div class="summary-footer">
															<a class="text-uppercase">Students in Class | <?= $best_teacher['t_students'] ?></a>
														</div>
													</div>
												</div>
											</div>
										...
										<section class="panel">
											<div class="panel-body bg-secondary">
												<div class="widget-summary">
													...
													<div class="widget-summary-col">
														<div class="summary">
															<h4 class="title">Lost Amount</h4>
															<div class="info">
																<strong class="amount">Cost  <?= $cost['cost'] ?></strong>
															</div>
														</div>
														<div class="summary-footer">
															<a class="text-uppercase">Lost Books | <?= $damage['damages'] ?></a>
														</div>
													</div>
												</div>
											</div>
