...
<?php
	...
	$query = "SELECT * FROM tbl_books ORDER BY bk_no ASC";
	$books = mysqli_query($db, $query);

	if ($books) {
		while ($book = mysqli_fetch_assoc($books)) {
				
			$books_list .=	"<tr style='font-size: 13px;'>";
			$books_list .=	"<td>{$book['bk_no']} . {$book['bk_name']} ({$book['bk_edition']}) - ({$book['bk_category_no']})</td>";
		 	$books_list .=	"<td>{$book['bk_author']} ({$book['bk_publisher']} - {$book['year']})</td>";
			$books_list .=	"<td>{$book['bk_qty']}</td>";
			$books_list .=	"<td class='actions'>";
			// $books_list .=	"<a href='#' class='hidden on-editing save-row'><i class='fa fa-save'></i></a>";
			// $books_list .=	"<a href='#' class='hidden on-editing cancel-row'><i class='fa fa-times'></i></a>";
			$books_list .=	"<a href='#edit{$book['bid']}' data-toggle='modal' ><i class='fa fa-pencil'></i></a>";
			$books_list .=	"<a href='#del{$book['bid']}' data-toggle='modal' ><i class='fa fa-trash-o'></i></a>";

			include('book_fetch.php');
					...
					<header class="page-header">
						...
						<div class="right-wrapper pull-right">
							<ol class="breadcrumbs">
								...
								<li><span>(Edit / Delete Book)</span></li>
							</ol>
					...
					<!-- start: page -->
						<section class="panel">
							<header class="panel-heading">
								...
								<h2 class="panel-title">Update Book's Details</h2>
							</header>
								...
								<table class="table table-bordered table-striped mb-none" id="datatable-editable">
									<thead>
										<tr style="font-size: 13px;">
											<th>Book No, Name with Edition & Category</th>
