...
<?php
	...
	$query = "SELECT *  FROM tbl_transactions t, tbl_books b, tbl_students s WHERE s.std_reg_no = t.std_no AND b.bk_no = t.bk_no AND t.t_status = 'no'";
	$book_returns = mysqli_query($db, $query);

	if ($book_returns) {
		while ($book_return = mysqli_fetch_assoc($book_returns)) {
			
			// ////////////////////////////////////////////
			$returned_list .= 	"<div class='isotope-item {$book_return['bk_category_no']} col-sm-6 col-md-4 col-lg-3'>";
			$returned_list .=	"<div class='thumbnail'>";
			$returned_list .=	"<div class='thumb-preview'>";
			$returned_list .=	"<a class='thumb-image' href='assets/images/projects/temp.jpg'>";
			$returned_list .=	"<img src='assets/images/projects/temp.jpg' class='img-responsive' alt='Project'>";
			$returned_list .=	"</a>";
			$returned_list .=	"<div class='mg-thumb-options'>";
			$returned_list .=	"<div class='mg-zoom'><i class='fa fa-search'></i></div>";
			$returned_list .=	"<div class='mg-toolbar'>";
			$returned_list .=	"<div class='mg-option checkbox-custom checkbox-inline'>";
			$returned_list .=	"<input type='checkbox' id='file_1' value='1'>";
			$returned_list .=	"<label for='file_1'>SELECT</label>";
			$returned_list .=	"</div>";
			$returned_list .=	"<div class='mg-group pull-right'>";
			$returned_list .=	"<a href='#'>EDIT</a>";
			$returned_list .=	"<button class='dropdown-toggle mg-toggle' type='button' data-toggle='dropdown'>";
			$returned_list .=	"<i class='fa fa-caret-up'></i>";
			$returned_list .=	"</button>";
			$returned_list .=	"<ul class='dropdown-menu mg-menu' role='menu'>";
			$returned_list .=	"<li><a href='#'><i class='fa fa-download'></i> Download</a></li>";
			$returned_list .=	"<li><a href='#'><i class='fa fa-trash-o'></i> Delete</a></li>";
			$returned_list .=	"</ul>";
			$returned_list .=	"</div>";
			$returned_list .=	"</div>";
			$returned_list .=	"</div>";
			$returned_list .=	"</div>";
			$returned_list .=	"<h5 class='mg-title text-semibold'>{$book_return['bk_name']}<small> edition : {$book_return['bk_edition']}</small></h5>";
			$returned_list .=	"<div class='mg-description'>";
			$returned_list .=	"<small class='pull-left text-muted'>Student : {$book_return['std_fname']} {$book_return['std_lname']}</small><br>";
			$returned_list .=	"<small class='pull-left text-muted'>Category : {$book_return['bk_category_no']}</small>";
			$returned_list .=	"<small class='pull-right text-muted'>Book No : {$book_return['bk_no']}</small><br>";
			$returned_list .=	"<small class='pull-left text-muted'>Teacher : {$book_return['teacher']}</small>";
			$returned_list .=	"<small class='pull-right text-muted'>Class : {$book_return['std_grade']} - {$book_return['std_class']}</small><br>";
			$returned_list .=	"<small class='pull-left text-muted'>Issued : {$book_return['bk_issued_date']}</small>";
			$returned_list .=	"<small class='pull-right text-muted'>Return : {$book_return['bk_should_return_date']}</small><br>";
			$returned_list .=	"<small class='pull-left text-muted'>Today : $curr_date</small>";

			if($curr_date > $book_return['bk_should_return_date']) {
				$date1=date_create($book_return['bk_should_return_date']); 
				$date2=date_create(date("Y-m-d")); 
				$diff=date_diff($date1,$date2);
				$days = $diff->format('%R%a days');
				$returned_list .=	"<small class='pull-right text-muted'>delay $days</small>";
			}

			else if ($curr_date < $book_return['bk_should_return_date']) {
				$date1=date_create($book_return['bk_should_return_date']); 
				$date2=date_create(date("Y-m-d")); 
				$diff=date_diff($date2,$date1);
				$days = $diff->format('%R%a days');
				$returned_list .=	"<small class='pull-right text-muted'>$days more to go</small>";
			}

			else {
				$returned_list .=	"<small class='pull-right text-muted'>Today Should Return</small>";
			}
			...
			<div class="progress">
				...
				<section role="main" class="content-body">
					<header class="page-header">
						<h2>View All Books Return Details</h2>
						...
						<div class="right-wrapper pull-right">
							<ol class="breadcrumbs">
								...
								<li><span>Return Books</span></li>
								<li><span>All Return Details</span></li>
							</ol>
